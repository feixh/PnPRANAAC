...
class P3PEstimator : public theia::Estimator< Match2D3D, Eigen::Matrix<double, 3, 4 > > {
public:
    P3PEstimator():
        Estimator< Match2D3D, Eigen::Matrix<double, 3, 4> >(),
        solver(){}
...
// Get the minimum number of samples needed to generate a model.
  ...
  virtual double Error(const Datum& data, const Model& model) const {
      // model is gwc
      const Eigen::Vector3d &worldPoint( data.worldPoint );
      Eigen::Vector3d proj( model.block<3,3>(0,0).transpose()*( worldPoint - model.block<3,1>(0,3) ) );
      if ( proj(2) < 0 ){
          return 1000000;
      }
      const Eigen::Vector3d &featureVector( data.featureVector );
